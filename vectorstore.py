...
class VectorStore:
    ...
    def upsert(self, path: str, summary: str, vector: np.ndarray, timestamp: float, sha256: str):
        b = to_bytes(vector)
        c = self.conn.cursor()
        c.execute('INSERT OR REPLACE INTO vectors (path, summary, embedding, timestamp, sha256) VALUES (?, ?, ?, ?, ?)',
                  (path, summary, b, timestamp, sha256))
        self.conn.commit()
    ...
    def all_embeddings(self) -> List[Tuple[int, str, str, np.ndarray, float]]:
        ...
        out = []
        for rid, path, summary, emb_blob, ts in rows:
            out.append((rid, path, summary, from_bytes(emb_blob), ts))
        return out
    ...
    def search(self, query_vector: np.ndarray, top_k=5):
        ...
        q_norm = np.linalg.norm(q) if np.linalg.norm(q) != 0 else 1e-12
        ...
        results = []
        for i in idx:
            results.append((float(scores[i]), {'id': int(ids[i]), 'path': paths[i], 'summary': summaries[i], 'timestamp': float(ts[i])}))
        return results
